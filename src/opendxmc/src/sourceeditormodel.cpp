...
/*
template<>
QVariant SourceItem<CTSource, std::array<double, 3>>::data(int role) const
{
	if (role == Qt::DisplayRole || role == Qt::EditRole)
	{
		auto n = f_data();
		QString data = QString::number(n[0]) + QString(", ") + QString::number(n[1]) + QString(", ") + QString::number(n[2]);
		return QVariant(data);
	}
	return QVariant();
}
template<>
void SourceItem<CTSource, std::array<double, 3>>::setData(const QVariant& data, int role)
{
	if (role ==Qt::DisplayRole || role == Qt::EditRole) {
		auto string = data.toString();
		auto stringList = string.split(",", QString::SkipEmptyParts);
...
template<>
QVariant SourceItem<CTSource, std::array<double, 6>>::data(int role) const
{
	if (role == Qt::DisplayRole || role == Qt::EditRole)
	{
		auto n = f_data();
		QString data = QString::number(n[0]) + QString(", ") +
			QString::number(n[1]) + QString(", ") +
			QString::number(n[2]) + QString(", ") +
			QString::number(n[3]) + QString(", ") +
			QString::number(n[4]) + QString(", ") +
			QString::number(n[5]);
		return QVariant(data);
	}
	return QVariant();
...
template<>
void SourceItem<CTSource, std::array<double, 6>>::setData(const QVariant& data, int role)
{
	if (role == Qt::DisplayRole || role == Qt::EditRole) {
		auto string = data.toString();
		auto stringList = string.split(",", QString::SkipEmptyParts);
